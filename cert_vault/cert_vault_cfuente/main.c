...
#define CERT_VAULT_REQUESTS_DIR "/ext/apps_data/cert_vault/requests"
#define CERT_VAULT_RESPONSES_DIR "/ext/apps_data/cert_vault/responses"
#define CERT_VAULT_IMPORT_EXTENSION ".fvp12"
#define CERT_VAULT_REQUEST_EXTENSION ".req"
#define CERT_VAULT_RESPONSE_EXTENSION ".resp"
#define CERT_VAULT_FORMAT "FVP12-1"
#define CERT_VAULT_REQUEST_FORMAT "FVP12-REQ-1"
#define CERT_VAULT_RESPONSE_FORMAT "FVP12-RESP-1"
#define CERT_VAULT_REQUEST_ID_SIZE 48u
#define CERT_VAULT_REQUEST_FILE_SIZE 128u
#define CERT_VAULT_HASH_SIZE 80u
#define CERT_VAULT_REASON_SIZE 256u
...
typedef enum {
    ...
    CertVaultActionPendingRequest,
    CertVaultActionApproveRequest,
    CertVaultActionRejectRequest,
...
typedef struct {
    char request_id[CERT_VAULT_REQUEST_ID_SIZE];
    char alias[CERT_VAULT_NAME_SIZE];
    char file_name[CERT_VAULT_REQUEST_FILE_SIZE];
    char field_name[CERT_VAULT_NAME_SIZE];
    char pdf_sha256[CERT_VAULT_HASH_SIZE];
    char reason[CERT_VAULT_REASON_SIZE];
    char created_at[CERT_VAULT_DATE_SIZE];
} CertVaultApprovalRequest;
...
static void cert_vault_reset_request(CertVaultApprovalRequest* request) {
    memset(request, 0, sizeof(CertVaultApprovalRequest));
}
...
static bool cert_vault_read_request_metadata(const char* path, CertVaultApprovalRequest* request) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(storage);
    char header[CERT_VAULT_HEADER_BUFFER_SIZE];
    bool ok = false;
    ...
    cert_vault_reset_request(request);
        ...
        while(*cursor != '\0') {
            ...
            *separator = '\0';
            ...
            if(strcmp(key, "format") == 0) {
                format_ok = strcmp(value, CERT_VAULT_REQUEST_FORMAT) == 0;
            } else if(strcmp(key, "request_id") == 0) {
                cert_vault_copy_string(request->request_id, sizeof(request->request_id), value);
            } else if(strcmp(key, "alias") == 0) {
                cert_vault_copy_string(request->alias, sizeof(request->alias), value);
            } else if(strcmp(key, "file_name") == 0) {
                cert_vault_copy_string(request->file_name, sizeof(request->file_name), value);
            } else if(strcmp(key, "field_name") == 0) {
                cert_vault_copy_string(request->field_name, sizeof(request->field_name), value);
            } else if(strcmp(key, "pdf_sha256") == 0) {
                cert_vault_copy_string(request->pdf_sha256, sizeof(request->pdf_sha256), value);
            } else if(strcmp(key, "reason") == 0) {
                cert_vault_copy_string(request->reason, sizeof(request->reason), value);
            } else if(strcmp(key, "created_at") == 0) {
                cert_vault_copy_string(request->created_at, sizeof(request->created_at), value);
            }
        }
    }
    ...
    ok = format_ok && request->request_id[0] != '\0';
    return ok;
}
...
static bool cert_vault_ensure_storage_dirs(void) {
    ...
    bool ok = storage_simply_mkdir(storage, "/ext/apps_data") &&
              ...
              storage_simply_mkdir(storage, CERT_VAULT_REQUESTS_DIR) &&
              storage_simply_mkdir(storage, CERT_VAULT_RESPONSES_DIR);
    furi_record_close(RECORD_STORAGE);
    return ok;
}
...
static bool cert_vault_build_response_path(
    const char* request_id,
    char* output,
    size_t output_size) {
    const int length = snprintf(
        output, output_size, "%s/%s%s", CERT_VAULT_RESPONSES_DIR, request_id, CERT_VAULT_RESPONSE_EXTENSION);
    return length > 0 && (size_t)length < output_size;
}

static bool cert_vault_find_first_pending_request(char* output, size_t output_size) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    File* directory = storage_file_alloc(storage);
    char file_name[CERT_VAULT_REQUEST_FILE_SIZE];
    char best_name[CERT_VAULT_REQUEST_FILE_SIZE];
    bool found = false;

    best_name[0] = '\0';

    if(cert_vault_ensure_storage_dirs() && storage_dir_open(directory, CERT_VAULT_REQUESTS_DIR)) {
        while(storage_dir_read(directory, NULL, file_name, sizeof(file_name))) {
            char request_id[CERT_VAULT_REQUEST_ID_SIZE];
            char response_path[CERT_VAULT_PATH_SIZE];

            if(!cert_vault_string_ends_with(file_name, CERT_VAULT_REQUEST_EXTENSION)) {
                continue;
            }

            cert_vault_copy_string(request_id, sizeof(request_id), file_name);
            request_id[strlen(request_id) - strlen(CERT_VAULT_REQUEST_EXTENSION)] = '\0';

            if(!cert_vault_build_response_path(request_id, response_path, sizeof(response_path))) {
                continue;
            }

            if(storage_file_exists(storage, response_path)) {
                continue;
            }

            if(!found || strcmp(file_name, best_name) < 0) {
                cert_vault_copy_string(best_name, sizeof(best_name), file_name);
                found = true;
            }
        }
    }
    ...
    if(found) {
        snprintf(output, output_size, "%s/%s", CERT_VAULT_REQUESTS_DIR, best_name);
    }
...
static bool cert_vault_show_first_pending_request(CertVaultApp* app) {
    char request_path[CERT_VAULT_PATH_SIZE];
    CertVaultApprovalRequest request;

    cert_vault_status_reset(app);

    if(!cert_vault_find_first_pending_request(request_path, sizeof(request_path))) {
        cert_vault_status_append(app, "No hay solicitudes pendientes.");
        return false;
    }

    if(!cert_vault_read_request_metadata(request_path, &request)) {
        cert_vault_status_append(app, "No se pudo leer la solicitud pendiente.\n%s", request_path);
        return false;
    }

    cert_vault_status_append(app, "Solicitud pendiente\n\nID: %s", request.request_id);
    if(request.alias[0] != '\0') {
        cert_vault_status_append(app, "\nAlias: %s", request.alias);
    }
    if(request.file_name[0] != '\0') {
        cert_vault_status_append(app, "\nPDF: %s", request.file_name);
    }
    if(request.reason[0] != '\0') {
        cert_vault_status_append(app, "\nMotivo: %s", request.reason);
    }
    if(request.field_name[0] != '\0') {
        cert_vault_status_append(app, "\nCampo: %s", request.field_name);
    }
    if(request.pdf_sha256[0] != '\0') {
        cert_vault_status_append(app, "\nSHA256: %.20s...", request.pdf_sha256);
    }
    if(request.created_at[0] != '\0') {
        cert_vault_status_append(app, "\nCreada: %s", request.created_at);
    }
    return true;
}

static bool cert_vault_respond_first_pending_request(CertVaultApp* app, const char* decision) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(storage);
    char request_path[CERT_VAULT_PATH_SIZE];
    char response_path[CERT_VAULT_PATH_SIZE];
    char response_body[512u];
    CertVaultApprovalRequest request;
    bool ok = false;
    ...
    if(!cert_vault_build_response_path(request.request_id, response_path, sizeof(response_path))) {
        cert_vault_status_append(app, "No se pudo construir la respuesta.");
        goto cleanup;
    }
    ...
    snprintf(
        ...
        "format=%s\nrequest_id=%s\ndecision=%s\ndecided_at=tick:%lu\ndevice=flipper\n",
        CERT_VAULT_RESPONSE_FORMAT,
        request.request_id,
        decision,
        (unsigned long)furi_get_tick());

    if(!storage_file_open(file, response_path, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        cert_vault_status_append(app, "No se pudo abrir el fichero de respuesta.");
        goto cleanup;
    }

    const size_t body_size = strlen(response_body);
    if(storage_file_write(file, response_body, body_size) != body_size || !storage_file_sync(file)) {
        cert_vault_status_append(app, "No se pudo guardar la respuesta.");
        storage_file_close(file);
        goto cleanup;
    }
    storage_file_close(file);

    cert_vault_status_append(app, "Solicitud %s\n\nID: %s", strcmp(decision, "approved") == 0 ? "aprobada" : "rechazada", request.request_id);
    if(request.file_name[0] != '\0') {
        cert_vault_status_append(app, "\nPDF: %s", request.file_name);
    }
    cert_vault_status_append(app, "\nRespuesta: %s", response_path);
    ok = true;
...
static void cert_vault_refresh_menu(CertVaultApp* app) {
    ...
    submenu_add_item(
        app->submenu, "1a pendiente", CertVaultActionPendingRequest, cert_vault_menu_callback, app);
    submenu_add_item(
        app->submenu, "Aprobar 1a", CertVaultActionApproveRequest, cert_vault_menu_callback, app);
    submenu_add_item(
        app->submenu, "Rechazar 1a", CertVaultActionRejectRequest, cert_vault_menu_callback, app);
...
static void cert_vault_menu_callback(void* context, uint32_t index) {
    ...
    if(index == CertVaultActionPendingRequest) {
        cert_vault_show_first_pending_request(app);
        cert_vault_show_text(app, app->status);
        return;
    }

    if(index == CertVaultActionApproveRequest) {
        cert_vault_respond_first_pending_request(app, "approved");
        cert_vault_show_text(app, app->status);
        return;
    }

    if(index == CertVaultActionRejectRequest) {
        cert_vault_respond_first_pending_request(app, "rejected");
        cert_vault_show_text(app, app->status);
        return;
    }
    ...
    cert_vault_status_append(
        app,
        "Cert Vault\n\n1. Convierte el .p12/.pfx en el PC con tools/p12_to_flipper_bundle.py\n2. Copia el .fvp12 a la SD\n3. Instala el bundle desde esta app\n4. Revisa la 1a solicitud pendiente y apruebala o rechazala\n\nRol actual del Flipper:\n- guardar e inventariar identidades\n- aprobar o rechazar solicitudes de firma\n- preparar el siguiente paso de modo token\n\nEl PDF firmado se genera en el PC tras la aprobacion.");
    cert_vault_show_text(app, app->status);
}
